...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        self.name = name
        self.exit = exit # The Place reached by exiting this Place (may be None).
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant:
            if QueenAnt.original is insect:
                return
            # Special Handling for BodyguardAnt and QueenAnt
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while pos.entrance is not None:
            counter += 1
            if counter <= self.min_range or counter >= self.max_range:
                pos = pos.entrance
                continue
            if len(pos.bees) > 0:
                hit = True
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                if insect.ant.watersafe is True:
                    self.ant = insect
                    insect.place = self
                else:
                    insect.armor = 0
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        if QueenAnt.original is None:
            QueenAnt.original = self
    ...
    def action(self, colony):
        ...
        """
        if QueenAnt.original is not self:
            self.reduce_armor(1)
        else:
            pos = self.place.exit
            ...
            while pos.exit is not None:
                if pos.ant is not None:
                    if not pos.ant in QueenAnt.double:
                        pos.ant.damage *= 2
                        if pos.ant.container == True:
                            pos.ant.ant.damage *= 2
                            QueenAnt.double += [pos.ant.ant]
                        QueenAnt.double += [pos.ant]
                pos = pos.exit
